...
echo "${boldwhite}Please note: This script is designed to work with dudestack and cape. See https://github.com/digitoimistodude${txtreset} "
...
STARTERTHEMEPATH="${HOME}/cape"
PROJECTTHEMEPATH="${HOME}/Projects/${PROJECTNAME}/content/themes/${THEMENAME}"

echo "${yellow}Checking cape updates...${txtreset}"
cd $HOME
git clone https://github.com/digitoimistodude/cape
cd $STARTERTHEMEPATH
git pull
...
# THE magical sed command by rolle (goes through every single file in theme folder and searchs and replaces every cape instance with THEMENAME):
for i in `grep -rl cape * 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;cape;${THEMENAME};" $i $i; done
for i in `grep -rl Cape * 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;Cape;${THEMENAME};" $i $i; done
